...
export default function NewRequestForm({ zones }: { zones: ServiceZone[] }) {
  ...
  const isValid =
    form.caller_full_name.trim() !== "" &&
    form.caller_phone.trim() !== "" &&
      ...
      <form className="rounded-xl border border-gray-200 bg-white p-6 flex flex-col gap-5">
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            onChange={(e) => setForm({ ...form, caller_full_name: e.target.value })}
        ...
        {!isValid && (
          <p className="text-xs text-gray-400">יש למלא שם, טלפון, כתובת איסוף ויעד לפני השליחה</p>
        )}
      </form>
    </div>
  );
