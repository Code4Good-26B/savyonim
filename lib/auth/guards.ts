...
export async function requireApprovedBearerAuth(request: Request): Promise<GuardResult> {
  ...
  if (user.status !== "approved") {
    return {
      ok: false,
      error: user.status === "pending" ? "Account pending approval" : "Account not active",
      httpStatus: 403,
    };
  }
...
export async function requireRepresentativeOrAdminAuth(request: Request): Promise<GuardResult> {
  const result = await requireApprovedBearerAuth(request);
